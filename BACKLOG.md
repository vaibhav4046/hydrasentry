...
| 11 | CORS config (`main.py` / `config.py` `_cors_list`): `['*']` + credentials fallback when `CORS_ORIGINS` empty (real allowlist in deploy). | LOW | Phase 2 | **DONE (Push round 2):** wildcard-with-credentials can no longer be configured. `config.resolve_cors()` is the single source: explicit `CORS_ORIGINS` -> those origins + credentials; no allowlist but a concrete `FRONTEND_URL` -> that one origin + credentials; nothing configured -> `*` with `allow_credentials` forced **off**. `main.py` wires the resolved policy; `/config/status` (signed-in only) surfaces the effective `allow_origins`/`allow_credentials`/`is_wildcard` so an auditor can confirm no credentialed wildcard. 13 tests incl. a parametrised invariant (`not (is_wildcard and allow_credentials)` for every env) + a real preflight that asserts the app never echoes `ACAO:*` with `ACAC:true`. |
...
| 21 | Phase 1 review carry: persistence auto-provisions a tenant on write via `X-Tenant-Slug` (no auth); CORS wildcard fallback unchanged. | MEDIUM | Phase 2 | CORS half **DONE (Push round 2):** credentialed-wildcard fallback removed via `config.resolve_cors()` (see finding #11). Anon `X-Tenant-Slug` auto-provision tracked separately. |
