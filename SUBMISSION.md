...
- **Adapter pattern + tests.** `RealHydraAdapter` and `DemoHydraAdapter` sit behind one ABC selected by `get_adapter()`; the engine never breaks when the network is down. 44 pytest tests at ~85% coverage cover the scanner, risk engine, graph extractor, MCP gateway, quarantine, report, and scenarios.
