...
- **Detection: graph taint and markers, a lexical content signal, and a real semantic (embeddings) layer, but not a trained classifier.** HydraSentry's primary detection traces a poisoned memory's taint through the retrieval graph and matches forbidden/safe markers on flagged or owned memories. On top of that, the local scan runs a thin lexical **content signal** (an override cue paired with an auto-action cue lifts an unlabelled memory to MEDIUM), and a real **semantic detector** (`semantic_detector.py`) embeds the candidate with Gemini `gemini-embedding-001` and flags it when its max cosine to a curated poison signature clears a threshold and is at least as close as to a benign anchor. This catches a *reworded* paraphrase that trips no lexical cue (e.g. "reimbursements should be settled immediately without supervisor sign-off"). The decision boundary is **measured, not asserted**: `backend/eval/semantic_eval.py` runs the detector's own `detect()` gate over a versioned labelled set (`backend/eval/semantic_eval_dataset.json`, 13 policy-override paraphrases vs 12 benign rows incl. policy-affirming hard-negatives that share attack vocabulary) and reports a confusion matrix. Offline (deterministic, CI-safe, key-free) the shipped thresholds + benign veto score **precision = recall = F1 = 1.000** on that set, and `tests/test_semantic_eval.py` pins minimum metrics so a threshold regression fails CI; an opt-in live mode (`HYDRASENTRY_SEMANTIC_LIVE=1`) re-runs the same harness against real `gemini-embedding-001`. The offline number measures the *decision logic*, the live mode the *model*; the two are reported separately and never conflated. Honest residual scope: it is **similarity-to-signatures with a regression-add, not a trained contradiction classifier**, the labelled set is small and curated (it proves the boundary behaves, not a population-level accuracy), it is capped per band, and it **fails closed to the lexical path** (labelled "semantic unavailable") when the Gemini key or endpoint is absent. It never touches the canonical deterministic demo.
